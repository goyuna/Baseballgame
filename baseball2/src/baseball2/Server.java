			...
			try {
				...
				while(status) {
					...
					if(rmsg[1].equals("logout")) {
						...
						msgSendAll("server/" + rmsg[0] + "님이 로그아웃했습니다.");
					...
					else if (rmsg[1].equals("game start")) {
						msgSendAll("server/"+rmsg[0]+"님이 게임을 시작하셨습니다. 숫자 3개를 입력하세요.");
						n++;
						game.randomInt();
					}
					...
					{
						if (n==1) n++;
						else if (rmsg[1].length() != 3)
						{
							msgSendAll("server/" + rmsg[1]);
							msgSendAll("server/ [" + rmsg[0] + "] 숫자를 다시 입력하세요.");
						}
						
						else 
						{
							game.inputUserNumber(rmsg[1]);
						    ...
						    val = game.getValue(); //종료처리
						    if(val) out = "success";
						    else out = "fail";
						    
						    msgSendAll("server/ [" + rmsg[0] + "] 게임 현황: " + strike + "S " + ball + "B" + " "+ out +" "+cnt1);
						    
							n++;
							
							
							if (val) {
								msgSendAll("server/" + "Game finish! " + rmsg[0] + " win!!");
								n=0;
							}
							else if (n==7) {
								msgSendAll("server/ Game finish! " + rmsg[0] + " lose!!");
								n=0;
							}
							else {
								msgSendAll("server/ [" + rmsg[0] + "] 숫자 3개를 입력하세요.");
							}
						}
					}
